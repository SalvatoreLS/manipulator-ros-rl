...
from distance_based_rl.agent.sac_agent import GaussianPolicy, ReplayBuffer, SACAgent
from distance_based_rl.agent.config import TrainingConfig


class TestGaussianPolicy:
    """Test the tanh-squashed Gaussian policy network."""

    def test_gaussian_policy_initialization(self):
        """Test that GaussianPolicy network initializes correctly."""
        state_dim = 6
        action_dim = 7
        network = GaussianPolicy(state_dim, action_dim, hidden_dim=256)
    ...
    def test_gaussian_policy_forward_pass(self):
        """Test that GaussianPolicy forward pass produces correct output shapes."""
        state_dim = 6
        action_dim = 7
        batch_size = 4
    ...
    def test_gaussian_policy_single_state(self):
        """Test GaussianPolicy with single state vector."""
        state_dim = 6
        action_dim = 7

        network = GaussianPolicy(state_dim, action_dim)
